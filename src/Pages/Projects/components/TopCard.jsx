...
const TopCard = ({ item }) => {
    return (
        <div className="lg:h-96 w-full h-80 overflow-hidden lg:p-4 rounded-t-md transition-opacity duration-500 relative">
            ...
            <div className={`lg:flex hidden absolute justify-center items-center h-full opacity-0 lg:group-hover:opacity-100 duration-500 z-10 top-0 left-0 text-caribbean_500 w-full`}>
            ...
            {
                item.github && (
                    <a href={item.github} target="_blank" rel="noopener noreferrer" className='absolute flex justify-center items-center right-2 bottom-0 h-10 w-10 bg-caribbean_500 rounded-full hover:h-12 hover:w-12 z-20 transtion duration-500 cursor-pointer'>
                        <FaGithub className='h-2/3 w-2/3 text-nobel_400' />
                    </a>
                )
            }
        </div>
