    ...
    return (
        <>
            ...
            <ul className={`padd:hidden flex flex-col h-screen w-screen flex-1 -left-8 transition-all absolute flex-grow duration-0 ease-in  ${open ? '-top-[200%]' : 'top-[-100vh] delay-[1500ms]'}`}>
                <div className={`flex-1 flex-grow h-1/2 flex-col flex justify-end items-end bg-dark_500 transtion ${open ? 'w-full duration-500' : 'w-0 delay-1000 duration-500'}`}>
                    {
                        links.map((link) => (
                ...
                <div className={`bg-dark_500 h-1/2 flex justify-center items-center ${open ? 'delay-500 w-[100vw] duration-500' : 'w-0 duration-500 delay-500'}`}>
